...
export default function PostersAdmin() {
  ...
  }, [session, currentPage, searchTerm, categoryFilter, sizeFilter, sortBy, sortOrder]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        alert(editingPoster ? 'Poster updated successfully!' : 'Poster created successfully!');
        setShowForm(false);
  ...
  const handleEdit = (poster) => {
    ...
    setFormData({
      title: poster.title || { english: '', tamil: '' },
      description: poster.description || { english: '', tamil: '' },
      category: poster.category || '',
      size: poster.size || '',
      price: poster.price?.toString() || '',
      discountPrice: poster.discountPrice?.toString() || '',
      imageUrl: poster.imageUrl || '',
      tags: poster.tags || [],
      featured: poster.featured || false,
      inStock: poster.inStock !== undefined ? poster.inStock : true,
      stockQuantity: poster.stockQuantity?.toString() || ''
    });
    setShowForm(true);
  };
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {posters.map((poster) => (
                  <tr key={poster.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-16 w-16">
                          {poster.imageUrl ? (
                            <img className="h-16 w-16 rounded-lg object-cover" src={poster.imageUrl} alt="" />
                          ) : (
                        ...
                        <div className="ml-4">
                          <div className="text-sm font-medium text-gray-900">
                            {poster.title?.english || 'Untitled'}
                          </div>
                          <div className="text-sm text-gray-500">
                            {poster.title?.tamil || 'தலைப்பு இல்லை'}
                          </div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {poster.discountPrice ? (
                          <>
                            <span className="line-through text-gray-500">₹{poster.price}</span>
                            <span className="ml-2 text-green-600 font-medium">₹{poster.discountPrice}</span>
                          </>
                        ) : (
                          <span className="font-medium">₹{poster.price}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {poster.inStock ? (
                          <span className="text-green-600">
                            {poster.stockQuantity ? `${poster.stockQuantity} units` : 'In Stock'}
                          </span>
                        ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {(poster.viewCount || 0).toLocaleString()}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex flex-col space-y-1">
                        ...
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                          poster.inStock ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}>
                          {poster.inStock ? 'Available' : 'Unavailable'}
                        </span>
                      </div>
                    </td>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., motivational, inspirational, tamil"
                />
              </div>
              ...
              <div className="flex justify-end space-x-3 pt-6 border-t">
                ...
                >
                  {submitting ? 'Saving...' : (editingPoster ? 'Update Poster' : 'Create Poster')}
                </button>
              </div>
            </form>
          </div>
