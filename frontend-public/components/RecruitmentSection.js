...
const RecruitmentSection = ({ entityId, entityType, language = 'en' }) => {
  ...
  const fetchActiveForms = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/forms?targetType=${entityType}&targetId=${entityId}&status=active&language=${language}`);
      if (response.ok) {
        const data = await response.json();
        // Enhance forms with additional status information
        const enhancedForms = (data.data || []).map(form => {
          const now = new Date();
          ...
          return {
            ...
            spotsLeft: form.settings?.responseLimit ? Math.max(0, form.settings.responseLimit - (form.responseCount || 0)) : null
          };
        });
  ...
  const openRecruitmentForm = async (form) => {
    try {
      ...
      const response = await fetch(`/api/forms/${form._id}?language=${language}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      // Add metadata
      submitData.append('entityId', entityId);
      submitData.append('entityType', entityType);
      ...
      submitData.append('formId', selectedForm._id);
      ...
      if (response.ok && result.success) {
        ...
        closeFormModal();
        // Refresh forms to update response counts
        fetchActiveForms();
      } else {
