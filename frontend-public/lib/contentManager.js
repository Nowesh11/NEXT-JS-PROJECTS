...
class ContentManager {
    ...
    async getAllContent() {
        try {
            // Use the correct API endpoint that exists on the backend
            ...
            if (!response.ok) {
                console.warn('Failed to load global content, trying sections endpoint');
                // Fallback to sections endpoint if global fails
                const sectionsResponse = await fetch(`${this.apiBaseUrl}/api/website-content/sections/home`, {
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    credentials: 'include'
                });
                if (!sectionsResponse.ok) throw new Error('Failed to load content from both endpoints');
...
export function useContentManager() {
    ...
    const getContent = useCallback((key, fallback = '') => {
        ...
        // Ensure content.page and content.global are arrays
        const pageContent = Array.isArray(content.page) ? content.page : [];
        const globalContent = Array.isArray(content.global) ? content.global : [];
        
        const allContent = [...pageContent, ...globalContent];
